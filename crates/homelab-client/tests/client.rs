...
fn compatible_capabilities() -> Capabilities {
    Capabilities {
        api: ApiVersion { major: API_MAJOR, minor: 0 },
...
async fn incompatible_capabilities_stop_mutation_before_it_is_sent() {
        ...
        .route(
            ...
            get(|| async {
                success(
                    ...
                    "incompatible",
                    Capabilities {
                        api: ApiVersion { major: API_MAJOR + 1, minor: 0 },
                        compatible_cli_major: API_MAJOR,
                        operations: Vec::new(),
                    },
                )
    ...
    let error = client
        ...
        .create_request(
            "incompatible",
            &CreateMediaRequest { media_id: 100, media_type: MediaType::Movie },
        )
        .await
        .unwrap_err();
...
async fn non_json_error_body_is_redacted_from_decode_error() {
    ...
    let error = client
        .media()
        .search("redacted", &SearchMediaQuery { query: "Alien".into() })
        .await
        .unwrap_err();

    assert!(matches!(error, ClientError::Decode { status: StatusCode::INTERNAL_SERVER_ERROR, .. }));
...
async fn fixed_api(State(seen): State<Seen>, request: Request) -> Response {
    ...
    seen.lock().push(SeenRequest { method: method.clone(), uri, body });
...
async fn every_operation_uses_a_typed_fixed_route() {
    ...
    let app = Router::new().fallback(fixed_api).with_state(Arc::clone(&seen));
    let mut base_url = spawn(app).await;
    base_url.set_path("/api/v1");
    let client = HomelabClient::new(base_url, reqwest::Client::new()).unwrap();
    ...
    let _: OperationEnvelope<Vec<MediaSearchItem>> = media
        .search("typed-contract", &SearchMediaQuery { query: "Witch Hat".into() })
        .await
        .unwrap();
    let _: OperationEnvelope<MediaSearchItem> = media.item_details("typed-contract", "item/one").await.unwrap();
    let _: OperationEnvelope<Vec<MediaRequest>> = media
        .list_requests("typed-contract", &ListRequestsQuery { status: Some("pending review".into()) })
        .await
        .unwrap();
    let _: OperationEnvelope<Vec<DownloadItem>> = media
        .list_downloads("typed-contract", &ListDownloadsQuery { status: Some("downloading".into()) })
        .await
        .unwrap();
    let _: OperationEnvelope<LibraryStatus> = media.library_status("typed-contract").await.unwrap();
    let _: OperationEnvelope<Vec<ActiveSession>> = media.active_sessions("typed-contract").await.unwrap();
    let _: OperationEnvelope<MediaRequest> = media
        ...
        .await
        .unwrap();
    let _: OperationEnvelope<MediaOperation> = media.approve_request("typed-contract", "r/1").await.unwrap();
    let _: OperationEnvelope<MediaOperation> = media.decline_request("typed-contract", "r/1").await.unwrap();
    let _: OperationEnvelope<MediaOperation> = media.pause_download("typed-contract", "d/1").await.unwrap();
    let _: OperationEnvelope<MediaOperation> = media.resume_download("typed-contract", "d/1").await.unwrap();
    let _: OperationEnvelope<MediaOperation> = media
        .delete_download("typed-contract", "d/1", &DeleteDownloadQuery { delete_files: true })
        .await
        .unwrap();
    let _: OperationEnvelope<MediaOperation> = media.retry_download("typed-contract", "d/1").await.unwrap();
    let _: OperationEnvelope<MediaOperation> = media.refresh_library("typed-contract").await.unwrap();

    let seen = seen.lock();
    let routes: Vec<_> = seen.iter().map(|request| (request.method.clone(), request.uri.as_str())).collect();
    assert_eq!(
        routes,
        vec![
            (Method::GET, "/api/v1/health"),
            ...
            (Method::POST, "/api/v1/media/downloads/d%2F1/pause"),
            (Method::POST, "/api/v1/media/downloads/d%2F1/resume"),
            (Method::DELETE, "/api/v1/media/downloads/d%2F1?delete_files=true"),
            (Method::POST, "/api/v1/media/downloads/d%2F1/retry"),
            (Method::POST, "/api/v1/media/library/refresh"),
        ]
    );
    assert_eq!(seen[8].body, Some(json!({"media_id": 100, "media_type": "movie"})));
    assert!(seen.iter().enumerate().all(|(index, request)| index == 8 || request.body.is_none()));
}
